...
def test_returned_letters_shows_count_of_recently_returned_letters(
    ...
    mocker.patch(
        ...
        return_value=[
            ...
            {
                'returned_letter_count': 10, 'reported_at': '2020-01-10'
            },
            # Just within the last 7 days (should be counted)
            {
                'returned_letter_count': 3, 'reported_at': '2020-01-3'
            },
            # Just after the last 7 days (should not be counted)
            {
                'returned_letter_count': 2, 'reported_at': '2020-01-2'
            },
        ],
    )
    page = client_request.get(
    ...
    assert normalize_spaces(
        banner.text
    ) == '13 returned letters latest report today'
    assert banner['href'] == url_for(
...
@pytest.mark.parametrize('reporting_date, expected_message', (
    ('2020-02-02', (
        '999 returned letters latest report today'
    )),
    ('2020-02-01', (
        '999 returned letters latest report yesterday'
    )),
    ('2020-01-31', (
        '999 returned letters latest report 2 days ago'
    )),
    ('2020-01-26', (
        '999 returned letters latest report 7 days ago'
    )),
    ('2020-01-25', (
...
def test_returned_letters_only_counts_recently_returned_letters(
    ...
    mocker.patch(
        ...
        return_value=[
            {  # A long time ago
                'returned_letter_count': 999, 'reported_at': reporting_date
            },
        ],
    )
    page = client_request.get(
